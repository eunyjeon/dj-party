...
//For public/ private, I think we need to introduce the concepts of friends
//Added a user belongsToMany user relationship, so that users can add friends
//This is so that people just can't join random rooms or add random people
const roomResolver = {
  Room: {
    users: async ({id}, args, {models}) => {
//Also added a flag for currentRoom in User table because the other way was rendering all users of a room and not the current ones...do we need the activeRoom flag in RoomUser?
      try {
        const users = await models.User.findAll({where: {currentRoom: id}})
        return users
      } catch (error) {
  ...
  Mutation: {
    ...
    addUserToRoom: async (parent, {spotifyUsername, roomId}, {models, getUser}) => {
      //after a user logs in, then the all rooms view only renders the public rooms and the rooms that they've been invited to- it's up to them to join or not.
      try {
        const creatorUserPromise = models.RoomUser.findOne({where: {roomId, userId: getUser(), activeRoom: true}})
        const addToRoomUserPromise = models.User.findOne({where: {spotifyUsername}})
        const [creatorUser, addToRoomUser] = await Promise.all([creatorUserPromise, addToRoomUserPromise])
        ...
        //This adds an instance in roomUser and defaults active room to be false
        await models.RoomUser.create({userId: addToRoomUser.id, roomId})
        return{
          ok: true
        }
